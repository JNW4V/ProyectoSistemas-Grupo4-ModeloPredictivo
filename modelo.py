...
def mostrar_grafico_btc():
    ...
    # Obtén la hora actual en la zona horaria local y la hora de hace 24 horas
    now = datetime.datetime.now(local_tz)
    start_time = now - datetime.timedelta(hours=24)

    # Convierte las fechas a timestamps en milisegundos (en UTC)
    ...
    # Convertir el timestamp a un formato legible en la zona horaria local
    ...
    plt.plot(df['timestamp'], df['close'], label='Precio BTC')

    plt.grid(True, which='both', linestyle='--', linewidth=0.5, color='gray')

    plt.xlabel('Hora')
    plt.ylabel('Precio de Cierre (USDT)')
    plt.title('Gráfico de precios de BTC/USDT (Últimas 24 horas)')
